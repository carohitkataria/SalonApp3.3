    ...
    <SupplierLayout>
      <div className="flex items-start justify-between mb-6 gap-3 flex-wrap">
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-1">Manage your catalog &amp; explore samples to add quickly.</p>
        </div>
        <Button onClick={openCreate} data-testid="add-product-btn" className="bg-amber-500 hover:bg-amber-600 text-black font-bold">
          <Plus className="w-4 h-4 mr-1" /> Add product
        </Button>
      </div>

      <div className="flex border border-zinc-800 rounded-lg overflow-hidden mb-5 text-xs font-bold max-w-md">
        <button
          onClick={() => setTab('catalog')}
          data-testid="tab-catalog"
          className={`flex-1 py-2.5 flex items-center justify-center gap-1.5 ${
            tab === 'catalog' ? 'bg-amber-500 text-black' : 'bg-transparent text-zinc-400 hover:text-white'
          }`}
        ...
        <button
          ...
          className={`flex-1 py-2.5 flex items-center justify-center gap-1.5 ${
            tab === 'samples' ? 'bg-amber-500 text-black' : 'bg-transparent text-zinc-400 hover:text-white'
          }`}
      ...
      {restockOpen && restockProduct && (
        <Modal title={`Restock — ${restockProduct.name}`} onClose={() => setRestockOpen(false)}>
          <p className="text-xs text-zinc-400 mb-3">Current available: <span className="text-white font-mono">{restockProduct.inventory_available}</span></p>
          <label className="text-[10px] uppercase tracking-widest font-bold text-zinc-400">Add quantity</label>
          <Input
            type="number"
            min="1"
            value={restockQty}
          ...
          <div className="flex justify-end gap-2 mt-5">
            ...
            <Button data-testid="restock-submit-btn" onClick={submitRestock} className="bg-amber-500 hover:bg-amber-600 text-black font-bold">
      ...
      {deleteId && (
        <Modal title="Remove product?" onClose={() => setDeleteId(null)}>
          <p className="text-sm text-zinc-300">This will hide the product from the marketplace. You can re-activate it later by editing.</p>
          <div className="flex justify-end gap-2 mt-5">
            <Button variant="outline" onClick={() => setDeleteId(null)}>Cancel</Button>
            <Button data-testid="confirm-delete-btn" onClick={confirmDelete} className="bg-rose-500 hover:bg-rose-600 text-white font-bold">
    ...
    <div>
      <div className="flex gap-2 mb-4 flex-wrap">
        ...
        <select
          ...
          className="bg-zinc-900 border border-zinc-800 text-white rounded-md px-3 py-2 text-sm"
        >
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-zinc-800 bg-zinc-900/40 p-10 text-center">
          ...
          <p className="text-sm text-zinc-400 mt-1">Start by browsing our 30 sample products — add any with one click.</p>
          <Button onClick={onSwitchSamples} className="mt-4 bg-amber-500 hover:bg-amber-600 text-black font-bold">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {products.map((p) => (
            <div key={p.id} data-testid={`product-card-${p.id}`} className="rounded-2xl border border-zinc-800 bg-zinc-900/40 overflow-hidden hover:border-amber-500/40 transition-colors flex flex-col">
              <div className="aspect-square bg-zinc-950 relative">
                {p.images?.[0] ? (
                  <img src={p.images[0]} alt={p.name} className="w-full h-full object-cover" />
                ) : (
                  <div className="w-full h-full flex items-center justify-center"><Boxes className="w-12 h-12 text-zinc-700" /></div>
                )}
                {!p.is_active && (
                  <div className="absolute top-2 left-2 px-2 py-0.5 rounded-full bg-zinc-800/90 border border-zinc-700 text-[10px] font-bold uppercase tracking-widest text-zinc-300">Draft</div>
                )}
                {p.is_low_stock && (
              ...
              <div className="p-3 flex-1 flex flex-col">
                <div className="text-[10px] uppercase tracking-widest font-bold text-amber-400">{p.category}{p.sub_category ? ` · ${p.sub_category}` : ''}</div>
                <h3 className="text-sm font-bold text-white mt-1 line-clamp-2">{p.name}</h3>
                {p.brand && <div className="text-xs text-zinc-500">{p.brand}{p.pack_size ? ` · ${p.pack_size}` : ''}</div>}
                <div className="flex items-center justify-between mt-2">
                  <div className="text-sm font-bold text-amber-300 flex items-center"><IndianRupee className="w-3 h-3" />{Number(p.selling_price).toLocaleString('en-IN')}</div>
                  <div className="text-[11px] text-zinc-500 line-through">{fmt(p.mrp)}</div>
                </div>
                <div className="text-xs text-zinc-400 mt-1.5">
                  Stock: <span className="font-mono text-white">{p.inventory_available}</span>
                  {p.inventory_reserved > 0 && <span className="text-zinc-500"> (+{p.inventory_reserved} reserved)</span>}
                </div>
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {samples.map((s) => (
            <div key={s.id} data-testid={`sample-card-${s.id}`} className="rounded-2xl border border-zinc-800 bg-zinc-900/40 overflow-hidden hover:border-amber-500/40 transition-colors flex flex-col">
              ...
              <div className="p-3 flex-1 flex flex-col">
                <div className="text-[10px] uppercase tracking-widest font-bold text-amber-400 flex items-center gap-1">
                  <Tag className="w-3 h-3" /> {s.category}
                </div>
                <h3 className="text-sm font-bold text-white mt-1 line-clamp-2">{s.name}</h3>
                <div className="text-xs text-zinc-500">{s.brand_placeholder}{s.pack_size ? ` · ${s.pack_size}` : ''}</div>
                {s.description && <p className="text-[11px] text-zinc-400 mt-1 line-clamp-2">{s.description}</p>}
                <div className="flex items-center justify-between mt-2">
                  <div className="text-xs text-zinc-400">Suggested MRP</div>
                  <div className="text-sm font-bold text-amber-300">{fmt(s.suggested_mrp)}</div>
                </div>
                <Button
                  data-testid={`add-sample-${s.id}`}
                  onClick={() => onAdd(s)}
                  className="mt-3 w-full bg-amber-500 hover:bg-amber-600 text-black font-bold text-xs"
                >
    ...
    <Modal title={mode === 'create' ? 'Add new product' : 'Edit product'} onClose={onClose} wide>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <Field label="Name *" testid="editor-name">
          <Input value={form.name} onChange={(e) => setF('name', e.target.value)} placeholder="Sulfate-Free Shampoo" data-testid="editor-name-input" className="bg-zinc-950 border-zinc-800 text-white" />
        </Field>
        <Field label="Brand">
          <Input value={form.brand} onChange={(e) => setF('brand', e.target.value)} placeholder="Salon Pro" className="bg-zinc-950 border-zinc-800 text-white" />
        </Field>
        <Field label="Category *">
          <Input value={form.category} onChange={(e) => setF('category', e.target.value)} placeholder="haircare / tools / skincare …" data-testid="editor-category-input" className="bg-zinc-950 border-zinc-800 text-white" />
        </Field>
        <Field label="Sub-category">
          <Input value={form.sub_category} onChange={(e) => setF('sub_category', e.target.value)} placeholder="shampoo / scissors / mask" className="bg-zinc-950 border-zinc-800 text-white" />
        </Field>
        <Field label="Unit *">
          <select value={form.unit} onChange={(e) => setF('unit', e.target.value)} data-testid="editor-unit-select" className="w-full bg-zinc-950 border border-zinc-800 text-white rounded-md px-3 py-2 text-sm">
            {UNITS.map(u => <option key={u} value={u}>{u}</option>)}
          </select>
        </Field>
        <Field label="Pack size">
          <Input value={form.pack_size} onChange={(e) => setF('pack_size', e.target.value)} placeholder="500 ml / 1 unit" className="bg-zinc-950 border-zinc-800 text-white" />
        </Field>
        <Field label="MRP (₹) *">
          <Input type="number" min="1" value={form.mrp} onChange={(e) => setF('mrp', e.target.value)} data-testid="editor-mrp-input" className="bg-zinc-950 border-zinc-800 text-white" />
        </Field>
        <Field label="Selling price (₹) *">
          <Input type="number" min="1" value={form.selling_price} onChange={(e) => setF('selling_price', e.target.value)} data-testid="editor-selling-input" className="bg-zinc-950 border-zinc-800 text-white" />
        </Field>
        <Field label="GST %">
          <Input type="number" min="0" max="28" value={form.gst_percent} onChange={(e) => setF('gst_percent', e.target.value)} className="bg-zinc-950 border-zinc-800 text-white" />
        </Field>
        <Field label="HSN code">
          <Input value={form.hsn_code} onChange={(e) => setF('hsn_code', e.target.value)} placeholder="3305" className="bg-zinc-950 border-zinc-800 text-white font-mono" />
        </Field>
        <Field label="Available stock">
          <Input type="number" min="0" value={form.inventory_available} onChange={(e) => setF('inventory_available', e.target.value)} data-testid="editor-stock-input" className="bg-zinc-950 border-zinc-800 text-white" />
        </Field>
        <Field label="Low-stock threshold">
          <Input type="number" min="0" value={form.low_stock_threshold} onChange={(e) => setF('low_stock_threshold', e.target.value)} className="bg-zinc-950 border-zinc-800 text-white" />
        </Field>
        <Field label="Min order qty">
          <Input type="number" min="1" value={form.min_order_qty} onChange={(e) => setF('min_order_qty', e.target.value)} className="bg-zinc-950 border-zinc-800 text-white" />
        </Field>
        <Field label="Status">
          <button
            type="button"
            ...
            className={`w-full px-3 py-2 rounded-md border text-xs font-bold uppercase tracking-widest text-left flex items-center gap-2 ${
              form.is_active ? 'border-emerald-500/50 bg-emerald-500/10 text-emerald-300' : 'border-zinc-700 bg-zinc-950 text-zinc-400'
            }`}
          >
      ...
      <Field label="Description" className="mt-3">
        <textarea
          ...
          className="w-full bg-zinc-950 border border-zinc-800 text-white rounded-md px-3 py-2 text-sm"
        />
      </Field>

      <div className="flex justify-end gap-2 mt-5 pt-4 border-t border-zinc-800">
        <Button variant="outline" onClick={onClose} disabled={busy}>Cancel</Button>
        <Button onClick={onSave} disabled={busy} data-testid="editor-save-btn" className="bg-amber-500 hover:bg-amber-600 text-black font-bold">
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4 py-6 bg-black/70 backdrop-blur-sm" onClick={onClose}>
      <div
        ...
        className={`w-full ${wide ? 'max-w-3xl' : 'max-w-md'} max-h-[90vh] overflow-y-auto rounded-2xl border border-zinc-800 bg-zinc-950 p-5 shadow-2xl`}
      >
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-base font-bold text-white">{title}</h2>
          <button onClick={onClose} className="p-1.5 rounded-lg text-zinc-500 hover:text-white hover:bg-zinc-800" data-testid="modal-close-btn">
            <X className="w-4 h-4" />
          </button>
        </div>
        {children}
